...
public class NhanVienService : INhanVienService
{
    ...
    public Task<int> Create(NhanVienDto dto) =>
        _repo.Create(new { dto.MaNhanVien, dto.MaChiNhanh, dto.HoTen, GioiTinh = dto.GioiTinh ?? "Nam", dto.NgaySinh, dto.SoDienThoai, dto.Email, ChucVu = dto.ChucVu ?? "Stylist", LuongCoBan = dto.LuongCoBan ?? 0, MatKhau = dto.MatKhau ?? "123456", dto.HinhAnh });

    public Task<int> Update(string id, NhanVienDto dto) =>
        _repo.Update(id, new { id, dto.MaChiNhanh, dto.HoTen, dto.GioiTinh, dto.NgaySinh, dto.SoDienThoai, dto.Email, dto.ChucVu, LuongCoBan = dto.LuongCoBan ?? 0, TrangThai = dto.TrangThai ?? true, dto.HinhAnh });
